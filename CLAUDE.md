...
- `原因`：名稱 | 排序 | 啟用 | 計入統計。**setup() 不預設任何原因**（使用者自建）。**計入統計**（RS_STATS=4，空白/TRUE=計入、FALSE=排除）：標成 FALSE 的原因，其紀錄照樣扣庫存但完全不進 `getStats`（給「不知何時抽掉」這種對帳用紀錄）；設定頁每列有 chart 圖示開關切換。**刪除＝硬刪整列**（下面上移）；改名（`renameReason`）會**回溯更新所有月分頁的原因文字**，統計才不會分裂；排序由 `reorderReasons(names)` 依前端傳入順序重寫「排序」欄，設定頁用上下箭頭調。歷史紀錄存原因文字快照，硬刪不影響歷史。「常用預設」功能已移除（P_DEFAULT 欄保留但不再使用）。
...
- `地點`：名稱 | 排序 | 啟用 | 緯度 | 經度（同人物 + 可選座標）。記錄「在哪抽」用，單選。`紀錄` 加「地點」欄（R_PLACE=11，存名稱快照）。座標空白＝無座標。
- `設定` 另有 `地圖樣式`（key-value，`setSetting(key,val)` 寫入，`readSettings().mapStyle`）。
