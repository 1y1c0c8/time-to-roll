...
- **P2**：庫存。每盒支數／售價／剩餘支數；加熱菸彈與盒菸抽一根自動 −1，顯示「X 盒 Y 支」，某口味整體剩 1 支時卡片跳彩蛋「盒仔內賰尾枝」。捲菸「可同時多包使用中」的菸草包追蹤，每捲一支選是哪一包 +1，「這包用完」歸檔記「共捲 N 支、每支成本 = 售價/N」；開新包的按鈕文案用「Time to Roll」。
- **P3**：統計。頻率趨勢（日／週／月／年）+ 原因分佈 + 時段分佈 + 花費估算。

## 資料模型（試算表分頁）
- `設定`：key-value（時區、幣別、預設每盒支數）。讀 A:B。
- `原因`：名稱 | 排序 | 啟用。刪除＝啟用設 FALSE（軟刪，保留歷史）。
- `菸品`：id | 類別 | 名稱 | 每盒支數 | 售價 | 售價單位 | 剩餘支數 | 常用預設 | 狀態 | 建立時間。捲菸的每盒支數留空。刪除＝狀態設「停用」。
...
- 記錄：`addSmoke({productId,reason})`、`updateSmoke({id,month,reason?,productId?,timeMillis?})`、`deleteSmoke({id,month})`、`getMonthRecords(tab)`
