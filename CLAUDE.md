...
- **P3（已完成，可用）**：統計。segmented 日／週／月／年切換，同步更新三張圖：趨勢＝**垂直長條 + 移動平均折線**（Chart.js；MA 視窗 日7/週4/月3、年不畫線）；原因分佈＝**水平排行長條**（HTML，含 % ）；時段分佈＝**早午晚深夜 4 組**（早5–11/午11–17/晚17–23/深夜23–5）。頂部 metric：期間總支數、平均每天。日=近30天、週=近12週、月=近12月、年=近5年。**花費估算延後到下一版**。切到統計分頁即 `loadStats()` 重抓。
- **P4（未做）**：花費估算。
